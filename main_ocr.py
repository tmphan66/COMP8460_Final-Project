...
def build_chroma_db():
    ...
    for _, row in df.iterrows():
        # Felix change
        # We create a richer text that is easy to search
        page_content = f"Drug: {str(row['drugName']).strip()}\nCondition: {str(row['condition']).strip()}\nReview: {row['review']}"
        metadata = {
    ...
    print("Chroma vector database saved to {DB_PATH} folder.")
...
# Felix changes
print("Initializing standard vector store retriever (k=4)...")
rag_retriever = vector_db.as_retriever(search_kwargs={"k": 4})
...
def chroma_search(query:str) -> str:
    ...
    1.  To answer qualitative questions about user experiences, reviews, side effects, or feelings for a *known* drug (e.g., 'side effects of Mirtazapine').
    ...
    If a drug name was extracted from an image, you can use that name to search (e.g., 'reviews for Ibuprofen').
    """
    ...
    found_drugs = set()
    for doc in docs:
        found_drugs.add(doc.metadata.get('drugName', 'N/A'))

    reviews_text = "\n".join([f"Review (Rating: {doc.metadata.get('rating', 'N/A')} for {doc.metadata.get('drugName', 'N/A')}): {doc.page_content}" for doc in docs])
    return f"Found reviews for drugs related to your query, including: {', '.join(found_drugs)}. Here is the raw context for you to summarize:\n{reviews_text}"
...
def pandas_analysis(query:str) -> str:
    # Felix changes
    """
    Use this tool for analytical questions about numbers, averages, counts, totals, or 'top' lists.
    For example: 'what is the average rating for Ibuprofen', 'how many reviews for Lipitor', 'top 5 most common drugs'.
    Do NOT use this tool to find drugs for a condition. Use `chroma_search` for that.
    If a drug name was extracted from an image, you MUST use this tool to answer questions about its average rating, count, etc. 
...
# Felix changes
# tool 3: OCR Reader Tool
reader = easyocr.Reader(['en'])

@tool
def drug_name_from_image(image_path: str) -> str:
    """
    ***PRIMARY USE CASE:*** This tool MUST be used first if the user's query contains a file path to an image (e.g., 'images/label.jpg').
    Use this tool to extract text from an image. The input MUST be the file path.
    It returns the recognized text from the image.
    You must then look at this text, identify the drug name, and use that drug name to call ANOTHER tool (`pandas_analysis` or `chroma_search`) to answer the user's original question.
    """
    print(f"\n>> Calling OCR Tool with image path: {image_path}")
    if not os.path.exists(image_path):
        return f"Error: Image file not found at {image_path}. Please check the path."
    try:
        results = reader.readtext(image_path)
        recognized_text = " ".join([text for (bbox, text, prob) in results])
        
        # Return the recognized text for the LLM to parse
        return f"Recognized text from image: {recognized_text}\n"
        
    except Exception as e:
        return f"Error: Could not process image or extract text. {e}"

tools = [chroma_search, pandas_analysis, drug_name_from_image]
...
# Felix changes
# Use prompt from LangChain Hub to support OCR tool
print("Pulling standard ReAct prompt from LangChain Hub...")
...
def main():
    ...
    while True:
        ...
        if not query.strip():
            print("Please enter a query or type 'exit'.123")
            continue
